...
public class PA_Kick : MonoBehaviour, IPlayerAttack
{
    ...
    public IEnumerator StartAction(PlayerInstance player, CharacterInstance target) {
        ...
        BattleMessage.setMessage(player.Name + " attacked");
        player.MP -= 8;
        int damage = player.Strength / 2 + player.Strength/2 * player.focus;
        player.focus = 0;
